...
let BindingComponent = class BindingComponent {
    ...
    shareuserscreen(videoreceiver) {
        ...
        if (navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ video: true, audio: true })
                ...
                this.startvideostream(videoreceiver);
            })
                .catch((err0r) => {
    ...
    startvideostream(videoreceiver) {
        ...
        this.videoSocketURL.onopen = () => {
            console.log("Openened connection to videowebsocket");
        };
        this.videoSocketURL.onmessage = (msg) => {
        ...
        this.videoSocketURL.onclose = (closeEvent) => {
            console.log("inside onclose for websocket");
        };
        this.videoSocketURL.onerror = (errorEvent) => {
    ...
    setupwebsocket() {
        ...
        this.webSocketURL.onopen = (openEvent) => {
            console.log("inside onopen for websocket");
            console.log("WebSocket OPEN: " + JSON.stringify(openEvent, null, 4));
        };
        this.webSocketURL.onclose = (closeEvent) => {
            console.log("inside onclose for websocket");
        ...
        this.webSocketURL.onerror = (errorEvent) => {
            console.log("inside onerror for websocket");
            console.log("WebSocket ERROR: " + JSON.stringify(errorEvent, null, 4));
        };
        this.webSocketURL.onmessage = (messageEvent) => {
            console.log("inside onmessage for websocket");
            var wsMsg = JSON.parse(messageEvent.data);
            console.log(wsMsg);
            console.log(this.receiver + " " + this.senderName);
            if ((wsMsg.receiverName == this.receiver && wsMsg.senderName == this.senderName) || (wsMsg.receiverName == this.senderName && wsMsg.senderName == this.receiver)) {
                if (wsMsg.hasVideo) {
                    ...
                    this.callertext = wsMsg.payload;
                    this.videocaller = wsMsg.senderName;
                    this.audio.play();
                }
                else {
